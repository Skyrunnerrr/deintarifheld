...
| R-SESS | In-memory session registry (restart/horizontal unsafe) | High | Critical | M9 durable control-plane design | OPEN | BLOCKING_BEFORE_PRODUCTION |
| R-KILL | In-memory kill switch (same class as R-SESS) | High | Critical | M9 durable control-plane design | OPEN | BLOCKING_BEFORE_PRODUCTION |
| R-DUAL | Root Next = live public; packages/* = local Ops; web/api skeletons | High | High | M9 Canonical Runtime ADR | OPEN | BLOCKING_BEFORE_IMPLEMENTATION |
| R-MIG | Public+Ops SQL share `supabase/migrations`; drafts also under packages/db | Med | High | M9/M10 migration ownership ADR | OPEN | BLOCKING_BEFORE_IMPLEMENTATION |
