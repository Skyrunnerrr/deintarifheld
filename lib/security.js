  ...
  if (!script) {
    ...
    const loaded = new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new RecaptchaClientError('recaptcha-script-timeout'))
      }, recaptchaWaitTimeoutMs())
      script.onload = () => {
        if (settled) return
        settled = true
...
export async function getRecaptchaToken(action) {
  ...
  const token = await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (settled) return
      settled = true
      reject(new RecaptchaClientError('recaptcha-execute-timeout'))
    }, recaptchaWaitTimeoutMs())
    Promise.resolve()
      .then(() => enterprise.execute(siteKey, { action }))
      .then((value) => {
        if (settled) return
        settled = true
        clearTimeout(timer)
      ...
      .catch(() => {
        ...
        settled = true
        clearTimeout(timer)
        reject(new RecaptchaClientError('recaptcha-execute-failed'))
      })
  })
