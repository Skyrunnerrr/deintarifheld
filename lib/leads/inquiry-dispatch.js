...
async function finishExisting(io, supabase, row, { kind, duplicate, idempotent }) {
  ...
  if (row.mail_status !== 'failed' || !data?.inquiry_type) {
    return {
      status: 409,
      body: {
        ok: false,
        ...
        mailStatus: row.mail_status || 'unknown',
        customerConfirmation: 'skipped',
        code: 'mail-status-unknown',
      },
    }
  }
  const claim =
    kind === 'career'
      ? await io.claimCareerMailRetry(supabase, id)
      : await io.claimLeadMailRetry(supabase, id)
  if (claim.error) {
    return { status: 500, body: { ok: false, stored: true, mail: false, code: 'storage-failed', leadId: id, leadRef } }
  }
  if (!claim.claimed) {
    return {
      status: 202,
  ...
  return inquiryPublicResult({ mailResult, leadId: id, leadRef, duplicate, idempotent })
}
