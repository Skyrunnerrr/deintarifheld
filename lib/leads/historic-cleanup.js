/**
 * Historic soft-delete cleanup — dry-run by default.
 * Never invents legal_hold. Never prints email/payload PII.
 */
...
export async function dryRunHistoricSoftDeletes(supabase) {
  ...
  return { ok: true, error: null, dryRun: true, applied: false, leads, careers }
}

export function cleanupAuthorization({
  apply = false,
  explicitApply = false,
  productionRuntime = false,
} = {}) {
  if (!apply) return { allowed: true, mode: 'dry-run' }
  if (productionRuntime) return { allowed: false, mode: 'blocked-production' }
  if (!explicitApply) return { allowed: false, mode: 'blocked-no-explicit-auth' }
  return { allowed: true, mode: 'apply-authorized-nonprod' }
}
