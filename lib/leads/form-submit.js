...
const MAIL_CODES = new Set([
  'mail-send-failed',
  'mail-not-configured',
  'mail-status-unknown',
  'mail-retry-in-progress',
  'mail-pending-recovery',
  'mail-failed-final',
...
const COPY = {
  informal: {
    ...
    mail: 'Deine Angaben sind gespeichert. Die Benachrichtigung wird automatisch einmal nachgeholt. Bitte sende das Formular nicht erneut.',
  ...
  formal: {
    ...
    mail: 'Ihre Angaben sind gespeichert. Die Benachrichtigung wird automatisch einmal nachgeholt. Bitte senden Sie das Formular nicht erneut.',
...
export function mapLeadSubmitUserMessage({ status, code, thrown } = {}, { tone = 'informal' } = {}) {
  ...
  if (MAIL_CODES.has(code) || status === 202) return copy.mail
