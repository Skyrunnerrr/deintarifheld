...
<body>
  ...
  <script>
    ...
    function render(items, counts) {
      $('counts').textContent = counts.leads + ' Anfragen · ' + counts.careers + ' Bewerbungen'
      if (!items.length) {
        $('list').innerHTML = '<p class="empty">Keine Einträge.</p>'
        return
      }
      $('list').innerHTML = items.map((item) => {
        const ref = item.lead_ref || item.application_ref || ''
        const extra = rowsFromPayload(item.payload)
        return '<article class="card"><h2>' + esc(titleOf(item)) + '</h2>' +
          '<p class="meta">' + esc(ref) + ' · ' + esc(when(item.created_at)) + ' · Mail ' + esc(item.mail_status || '—') + '</p>' +
          '<table>' +
    ...
    async function load() {
      ...
      render(items, { leads: (json.leads || []).length, careers: (json.careers || []).length })
    }
    $('open').onclick = load
    $('reload').onclick = load
    $('logout').onclick = () => {
