...
import { productionPublicIntakeEnvProblems } from './production-env-preflight.js'
...
export async function enforcePublicIntake(request, options = {}) {
  if (isProductionRuntime()) {
    const configProblems = productionPublicIntakeEnvProblems()
    if (configProblems.length) {
      leadsLog('error', 'intake.production_config_invalid', {
        configProblemCount: configProblems.length,
      })
      return { ok: false, code: 'service-not-configured', status: 503 }
    }
  }
  ...
  // The dedicated smoke bypass exists only outside production and requires its
  // own secret. It must also bypass the submit quota so a full multi-form
  // preview E2E can exercise every surface without creating false 429s.
  if (!bypass) {
