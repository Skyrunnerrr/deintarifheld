...
const PUBLIC_INTAKE_PROBLEMS = new Set([
  'SUPABASE_URL',
  'SUPABASE_SERVICE_ROLE_KEY',
  'RECAPTCHA_PROJECT_ID',
  'RECAPTCHA_API_KEY',
  'NEXT_PUBLIC_RECAPTCHA_PUBLIC_KEY',
  'LEGACY_RECAPTCHA_PUBLIC_ENV',
  'LEADS_RATE_LIMIT_SALT',
  'LEADS_RATE_LIMIT_PROVIDER',
  'LEADS_ALLOW_MEMORY_RATE_LIMIT',
  'LEADS_ALLOW_SMOKE_BYPASS',
  'LEADS_ALLOWED_ORIGINS',
])

// Runtime intake is intentionally storage-first. Mail configuration remains a
// mandatory release/deployment gate, but a temporary mail problem must not turn
// every public form into a 503 before the lead can be persisted.


export function productionPublicIntakeEnvProblems(env = process.env) {
  return productionApiEnvProblems(env).filter((problem) => PUBLIC_INTAKE_PROBLEMS.has(problem))
}
