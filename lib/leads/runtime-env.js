...
export function effectiveAllowedOrigins() {
  // Production is intentionally non-extensible: an environment typo must not
  // widen the browser-origin trust boundary.
  if (isProductionRuntime()) return productionSiteOrigins()

  const configured = parseOriginList(process.env.LEADS_ALLOWED_ORIGINS)
  return [...new Set([...productionSiteOrigins(), ...localDevOrigins(), ...configured])]
}
