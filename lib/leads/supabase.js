...
const RECOVERY_LEAD_COLS =
  'id, lead_ref, status, payload, mail_status, mail_mode, created_at, updated_at'
const RECOVERY_CAREER_COLS =
  'id, application_ref, status, payload, mail_status, mail_mode, created_at, updated_at'

function recoveryListError() {
  return { data: [], error: { code: 'recovery-list-failed' } }
}

/** Failed unified-inquiry leads old enough for the single automatic retry. */
export async function listFailedInquiryLeads(supabase, { olderThanIso, limit = 50 } = {}) {
  if (!supabase || !olderThanIso) return recoveryListError()
  const { data, error } = await supabase
    .from('leads')
    .select(RECOVERY_LEAD_COLS)
    .eq('mail_status', 'failed')
    .neq('status', 'deleted')
    .lt('updated_at', olderThanIso)
    .in('payload->>inquiry_type', ['private_energy', 'business_energy', 'general'])
    .order('updated_at', { ascending: true })
    .limit(limit)
  if (error) return recoveryListError()
  return { data: data || [], error: null }
}

/** Failed partner inquiries on the existing career table. */
export async function listFailedInquiryCareers(supabase, { olderThanIso, limit = 50 } = {}) {
  if (!supabase || !olderThanIso) return recoveryListError()
  const { data, error } = await supabase
    .from('career_applications')
    .select(RECOVERY_CAREER_COLS)
    .eq('mail_status', 'failed')
    .neq('status', 'deleted')
    .lt('updated_at', olderThanIso)
    .eq('payload->>inquiry_type', 'partner')
    .order('updated_at', { ascending: true })
    .limit(limit)
  if (error) return recoveryListError()
  return { data: data || [], error: null }
