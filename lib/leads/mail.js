...
import { inquirySubjectLabel, isInquiryType } from './inquiry-contract.js'
...
const FROM_RE =
  /^(?:[A-Za-z0-9 ._-]{1,80}\s*)?<[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}>$|^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/
...
/** Authenticated From only. Display name form from env is allowed. No CRLF. */
export function safeFromAddress(value) {
  const clean = String(value || '').replace(/[\r\n\u0000-\u001F\u007F]/g, '').trim()
  if (!FROM_RE.test(clean)) return ''
  return clean
}

/** Reply-To is a bare validated address. User text never becomes a header. */
export function safeReplyTo(value) {
  const clean = stripMailControls(value, 180)
  return BARE_EMAIL_RE.test(clean) ? clean : ''
}
...
export function parseLeadToAddresses(value) {
  return String(value || '')
    ...
    .filter((part) => BARE_EMAIL_RE.test(part))
}

export function usesInquiryMail(data) {
  return isInquiryType(data?.inquiry_type)
}

export function shouldSendCustomerMail(data) {
  if (usesInquiryMail(data)) return false
  return customerMailDualGuardOpen()
}
...
export function buildInquiryOpsMail({ leadRef, data, submittedAt }) {
  const when = formatWhen(submittedAt)
  const label = inquirySubjectLabel(data.inquiry_type) || 'Anfrage'
  const who = stripMailControls(data.firma || data.name || 'Anfrage', 80)
  const subjectAdmin = stripMailControls(`${label} — ${who} — ${leadRef}`, 180)
  const lines = [
    `${label} ${leadRef}`,
    `Zeit: ${when}`,
    `Typ: ${data.inquiry_type || ''}`,
    `Name: ${data.name || ''}`,
    `E-Mail: ${data.email || ''}`,
    `Telefon: ${data.phone || ''}`,
    `PLZ: ${data.plz || ''}`,
    ...
    `Zusammenarbeit: ${data.motivation || ''}`,
    `Nachricht: ${data.nachricht || ''}`,
    `Quelle: ${data.source_page || ''}`,
    'Kundenbestätigung: bewusst übersprungen',
  ]
  return {
    templateIds: ['dth.inquiry.admin.v1'],
    subjectAdmin,
    adminText: lines.join('\n'),
    adminHtml: `
      <div style="font-family:Arial,sans-serif;line-height:1.5;color:#152033;">
        <h2 style="margin:0 0 12px;">${esc(label)}</h2>
        <p style="margin:0 0 16px;color:#5B6578;">Ref <strong>${esc(leadRef)}</strong> · ${esc(when)}</p>
        <table style="border-collapse:collapse;">
          ${row('Typ', data.inquiry_type)}
          ${row('Name', data.name)}
          ${row('E-Mail', data.email)}
          ${row('Telefon', data.phone)}
          ${row('PLZ', data.plz)}
          ${row('Firma', data.firma)}
          ${row('Verbrauch', data.verbrauch)}
          ${row('Tarifinfo', data.tarifinfo)}
          ${row('Zähler', data.zaehler)}
          ${row('Beschreibung', data.beschreibung)}
          ${row('Zusammenarbeit', data.motivation)}
          ${row('Nachricht', data.nachricht)}
          ${row('Quelle', data.source_page)}
        </table>
        <p style="margin:16px 0 0;font-size:12px;color:#7D8798;">Kundenbestätigung: übersprungen</p>
      </div>`,
...
export async function sendLeadEmails({ leadRef, data, submittedAt, channel = 'business' }) {
  ...
  if (mode === 'mock') {
    const bundle = usesInquiryMail(data)
      ? buildInquiryOpsMail({ leadRef, data, submittedAt })
      : resolveMailBundle({ channel, leadRef, data, submittedAt })
    return {
      ok: true,
      mode: 'mock',
      code: 'sandbox_accepted',
      mailStatus: 'accepted',
      customerConfirmation: usesInquiryMail(data) ? 'skipped' : 'n/a',
  ...
  if (mode === 'internal_live') {
    ...
    const from = safeFromAddress(process.env.LEADS_FROM_EMAIL)
    ...
    const mail = usesInquiryMail(data)
      ? buildInquiryOpsMail({ leadRef, data, submittedAt })
      : buildInternalOpsMail({ channel, leadRef, data, submittedAt })
    const resend = new Resend(apiKey)
    const internal = await resend.emails.send({
      from,
      to,
      replyTo: safeReplyTo(mail.replyTo) || undefined,
      subject: stripMailControls(mail.subjectAdmin, 180),
      text: mail.adminText,
      html: mail.adminHtml,
    })
    if (internal.error) {
  ...
  if (!shouldSendCustomerMail(data)) {
    return {
      ok: true,
      mode: 'live',
      mailStatus: 'internal_sent',
      customerConfirmation: usesInquiryMail(data) ? 'skipped' : 'blocked',
      templateIds: mail.templateIds,
      providerEmailId: extractProviderId(internal),
    }
  }
  ...
  const confirmation = await resend.emails.send({
    ...
    replyTo: to[0],
    subject: stripMailControls(mail.subjectCustomer, 180),
