...
function mailHeaderText(value, fallback = '') {
...
const MAIL_SUCCESS_STATUSES = new Set(['accepted', 'internal_sent', 'partial_failed'])

/**
 * Truthful mail fields from a stored lead/career row.
 * Never invent success for null/legacy mail_status.
 */
export function mailFieldsFromStored(row) {
  const rawStatus = typeof row?.mail_status === 'string' ? row.mail_status.trim() : ''
  const rawMode = typeof row?.mail_mode === 'string' ? row.mail_mode.trim() : ''
  const mailStatus = rawStatus || 'unknown'
  let customerConfirmation = 'n/a'
  if (rawStatus === 'partial_failed') customerConfirmation = 'failed'
  else if (rawMode === 'live' && rawStatus === 'accepted') customerConfirmation = 'sent'
  else if (rawMode === 'live' && rawStatus === 'internal_sent') customerConfirmation = 'blocked'
  else if (rawMode === 'internal_live') customerConfirmation = 'skipped'

  return {
    mail: MAIL_SUCCESS_STATUSES.has(rawStatus),
    mailMode: rawMode || null,
    mailStatus,
...
export function buildUnternehmenMails({ leadRef, data, submittedAt }) {
  ...
  const subjectAdmin = `[B2B] ${mailHeaderText(data.firma, 'Unternehmen')} — ${leadRef}`
...
export function buildPrivateMails({ leadRef, data, submittedAt }) {
  ...
  const subjectAdmin = `[Privat] ${mailHeaderText(name, 'Interessent')} — ${leadRef}`
...
export function buildCareerMails({ leadRef, data, submittedAt }) {
  const when = formatWhen(submittedAt)
  const subjectAdmin = `[Karriere] ${mailHeaderText(data.name, 'Bewerbung')} — ${leadRef}`
  const subjectCustomer = 'Deine Partneranfrage bei DeinTarifheld'

  const adminHtml = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#152033;">
      <h2 style="margin:0 0 12px;">Neue Partneranfrage</h2>
  ...
  const customerHtml = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#152033;">
      <h2 style="margin:0 0 12px;">Partneranfrage eingegangen</h2>
      <p>Hallo ${esc(data.name)},</p>
      <p>vielen Dank für dein Interesse an einer Zusammenarbeit mit DeinTarifheld. Wir haben deine Partneranfrage erhalten.</p>
  ...
  const customerText = [
    `Hallo ${data.name},`,
    '',
    'vielen Dank für dein Interesse an einer Zusammenarbeit mit DeinTarifheld. Wir haben deine Partneranfrage erhalten.',
    `Referenz: ${leadRef}`,
  ].join('\n')
...
export function parseLeadToAddresses(value) {
  ...
  if (!parts.length || parts.length > 10 || parts.some((part) => !part)) return []

  const simpleEmail = /^[^\s@,<>]+@[^\s@,<>]+\.[^\s@,<>]+$/
  if (parts.some((part) => !simpleEmail.test(part))) return []
...
function validFromAddress(value) {
  ...
  const address = raw.match(/<([^<>]+)>\s*$/)?.[1]?.trim() || raw
...
export function buildInternalOpsMail({ channel, leadRef, data, submittedAt }) {
  ...
  if (channel === 'private') {
    ...
    return {
      ...
      subjectAdmin: `[Privat] ${mailHeaderText(energyType, 'Tarif')}-Anfrage — ${mailHeaderText(name, 'Interessent')} — ${leadRef}`,
      adminText: [
  ...
  if (channel === 'career') {
    ...
    return {
      ...
      subjectAdmin: `[Partner] Anfrage — ${mailHeaderText(data.name, 'Interessent')} — ${leadRef}`,
      adminText: [
        `Neue Partneranfrage ${leadRef}`,
        `Zeit: ${when}`,
        `Name: ${data.name || ''}`,
  ...
  return {
    ...
    subjectAdmin: `[B2B] ${mailHeaderText(energyType, 'Energie')}-Anfrage — ${mailHeaderText(data.firma, 'Unternehmen')} — ${leadRef}`,
    adminText: [
...
export function resendSendTimeoutMs() {
  const raw = String(process.env.RESEND_SEND_TIMEOUT_MS ?? '').trim()
  if (!raw) return DEFAULT_RESEND_TIMEOUT_MS
  const value = Number(raw)
  if (!Number.isInteger(value) || value < 100 || value > 15000) return null
  return value
}

function resendIdempotencyKey(leadRef, kind) {
  const ref = String(leadRef || '').replace(/[^A-Za-z0-9_-]/g, '').slice(0, 80)
  const type = kind === 'customer' ? 'customer' : 'internal'
  return `dth-${ref || 'unknown'}-${type}`
}

async function sendResendEmail(resend, message, { idempotencyKey } = {}) {
  const timeoutMs = resendSendTimeoutMs()
  if (timeoutMs === null) {
    return { ok: false, errorCode: 'resend-timeout-config-invalid' }
  }
  ...
  try {
    ...
    if (controller?.signal?.aborted) {
      return { ok: false, errorCode: 'resend_timeout' }
    }
    if (result?.error) {
      return {
        ok: false,
        errorCode: typeof result.error.name === 'string' && result.error.name
          ? result.error.name.slice(0, 80)
          : 'resend_error',
      }
    }
    return {
      ok: true,
      providerEmailId: extractProviderId(result),
    }
  } catch {
    if (controller?.signal?.aborted) {
      return { ok: false, errorCode: 'resend_timeout' }
    }
    // Provider/transport exceptions must never escape after a lead is already stored.
    return { ok: false, errorCode: 'resend_transport_error' }
  } finally {
    if (timeout) clearTimeout(timeout)
  }
}

export function customerMailDualGuardOpen() {
  const mode = (process.env.LEADS_MAIL_MODE || 'mock').trim().toLowerCase()
  const allow = (process.env.ALLOW_CUSTOMER_MAIL || '').trim().toUpperCase()
  return mode === 'live' && allow === 'YES'
}
...
export async function sendLeadEmails({ leadRef, data, submittedAt, channel = 'business' }) {
  ...
  if (mode === 'internal_live') {
    ...
    if (!apiKey || !validFromAddress(from) || !to.length) {
      return {
        ok: false,
    ...
    const internal = await sendResendEmail(resend, {
      from,
      to,
      ...
      text: mail.adminText,
      html: mail.adminHtml,
    }, { idempotencyKey: resendIdempotencyKey(leadRef, 'internal') })
    if (!internal.ok) {
      return {
        ok: false,
        code: 'mail-send-failed',
        ...
        mailStatus: 'failed',
        customerConfirmation: 'skipped',
        providerErrorCode: internal.errorCode,
      }
    }
    return {
      ok: true,
      ...
      providerEmailId: internal.providerEmailId,
    }
  }
  ...
  if (!internal.ok) {
    return {
      ...
      customerConfirmation: 'n/a',
      providerErrorCode: internal.errorCode,
      internalDelivery: 'failed',
    }
  }

  if (!customerMailDualGuardOpen()) {
    return {
      ok: true,
      mode: 'live',
      mailStatus: 'internal_sent',
      customerConfirmation: 'blocked',
      templateIds: mail.templateIds,
      providerEmailId: internal.providerEmailId,
    }
  }

  const confirmation = await sendResendEmail(resend, {
    from,
  ...
  }, { idempotencyKey: resendIdempotencyKey(leadRef, 'customer') })
  if (!confirmation.ok) {
    return {
      ok: false,
      code: 'mail-send-failed',
      mode: 'live',
      // Internal notification already succeeded. Preserve that truth instead of
      // flattening the whole mail operation to a generic failed state.
      mailStatus: 'partial_failed',
      customerConfirmation: 'failed',
      providerErrorCode: confirmation.errorCode,
      providerEmailId: internal.providerEmailId,
      internalDelivery: 'sent',
    }
  }
  ...
  return {
    ...
    templateIds: mail.templateIds,
    providerEmailId: internal.providerEmailId,
    customerProviderEmailId: confirmation.providerEmailId,
    internalDelivery: 'sent',
  }
}
