...
/**
 * Minimized internal ops notification (temporary internal_live mode).
 * No honeypot/IP/UA/secrets; career motivation truncated.
 */
export function buildInternalOpsMail({ channel, leadRef, data, submittedAt }) {
  ...
  if (channel === 'private') {
    ...
    return {
      ...
      subjectAdmin: `[Privat/internal] ${name} — ${leadRef}`,
      adminText: [
        `Interne Privat-Benachrichtigung ${leadRef}`,
        `Zeit: ${when}`,
        `Name: ${data.firstName || ''}`,
        `E-Mail: ${data.email || ''}`,
        `Telefon: ${data.phone || ''}`,
      ...
      adminHtml: `
        <div style="font-family:Arial,sans-serif;line-height:1.5;color:#152033;">
          <h2 style="margin:0 0 12px;">Interne Privat-Benachrichtigung</h2>
          <p style="margin:0 0 16px;color:#5B6578;">Ref <strong>${esc(leadRef)}</strong> · ${esc(when)}</p>
          <table style="border-collapse:collapse;">
            ${row('Name', data.firstName)}
            ${row('E-Mail', data.email)}
  ...
  if (channel === 'career') {
    const motivation = String(data.motivation || '').slice(0, 280)
    return {
      templateIds: [MAIL_TEMPLATE_IDS.career_admin],
      subjectAdmin: `[Karriere/internal] ${data.name || 'Bewerbung'} — ${leadRef}`,
      adminText: [
        `Interne Karriere-Benachrichtigung ${leadRef}`,
        `Zeit: ${when}`,
        `Name: ${data.name || ''}`,
        `E-Mail: ${data.email || ''}`,
        `Telefon: ${data.phone || ''}`,
        `Motivation (gekürzt): ${motivation}`,
        'Kundenbestätigung: bewusst übersprungen (internal_live)',
      ].join('\n'),
      adminHtml: `
        <div style="font-family:Arial,sans-serif;line-height:1.5;color:#152033;">
          <h2 style="margin:0 0 12px;">Interne Karriere-Benachrichtigung</h2>
          <p style="margin:0 0 16px;color:#5B6578;">Ref <strong>${esc(leadRef)}</strong> · ${esc(when)}</p>
          <table style="border-collapse:collapse;">
            ${row('Name', data.name)}
            ${row('E-Mail', data.email)}
            ${row('Telefon', data.phone)}
            ${row('Motivation (gekürzt)', motivation)}
          </table>
  ...
  return {
    ...
    subjectAdmin: `[B2B/internal] ${data.firma || 'Unternehmen'} — ${leadRef}`,
    adminText: [
      `Interne B2B-Benachrichtigung ${leadRef}`,
      `Zeit: ${when}`,
      `Firma: ${data.firma || ''}`,
      `Ansprechpartner: ${data.ansprechpartner || ''}`,
      `E-Mail: ${data.email || ''}`,
    ...
    adminHtml: `
      <div style="font-family:Arial,sans-serif;line-height:1.5;color:#152033;">
        <h2 style="margin:0 0 12px;">Interne B2B-Benachrichtigung</h2>
        <p style="margin:0 0 16px;color:#5B6578;">Ref <strong>${esc(leadRef)}</strong> · ${esc(when)}</p>
        <table style="border-collapse:collapse;">
          ${row('Firma', data.firma)}
          ${row('Ansprechpartner', data.ansprechpartner)}
