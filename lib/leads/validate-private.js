...
export function validatePrivatePayload(raw) {
  ...
  if (
    ...
    exceedsMax(raw.firstName || raw.name, MAX.firstName) ||
    exceedsMax(raw.email, MAX.email) ||
    exceedsMax(raw.phone || raw.telefon, MAX.phone) ||
    exceedsMax(raw.provider || raw.versorger, MAX.provider) ||
    exceedsMax(raw.usage || raw.consumption || raw.verbrauch, MAX.usage) ||
    exceedsMax(raw.zip || raw.plz, MAX.zip) ||
    exceedsMax(raw.type || raw.energieart, MAX.type) ||
    exceedsMax(raw.source_page || raw.sourcePage || raw.page, MAX.source_page) ||
  ...
  if (!provider) {
    return { ok: false, code: 'provider-required' }
  }

  if (!/^\d+$/.test(usage) || Number(usage) <= 0) {
    return { ok: false, code: 'invalid-usage' }
  }
  ...
  if (!new Set(['strom', 'gas']).has(normalizedEnergyType)) {
