...
export function validateUnternehmenPayload(raw) {
  ...
  if (raw.page_source !== 'unternehmen') {
  ...
  if (
    exceedsMax(raw.page_source, MAX.page_source) ||
    ...
    exceedsMax(raw.energieart, MAX.energieart) ||
    exceedsMax(raw.verbrauchStrom, MAX.verbrauchStrom) ||
    exceedsMax(raw.verbrauchGas, MAX.verbrauchGas) ||
    exceedsMax(raw.standorte, MAX.standorte) ||
    exceedsMax(raw.versorger, MAX.versorger) ||
    exceedsMax(raw.vertragslaufzeit, MAX.vertragslaufzeit) ||
    exceedsMax(raw.nachricht, MAX.nachricht) ||
    exceedsMax(raw.source_page || raw.sourcePage, MAX.source_page) ||
    exceedsMax(String(raw.form_version || '2.0'), MAX.form_version)
  ) {
  ...
  if (!validPhone(telefon)) {
    return { ok: false, code: 'invalid-phone' }
  }

  const normalizedEnergyType =
    energieart.toLowerCase() === 'strom' ? 'Strom' :
    energieart.toLowerCase() === 'gas' ? 'Gas' :
    ''

  if (!normalizedEnergyType) {
    return { ok: false, code: 'invalid-energy-type' }
  }

  const normalizedStrom = normalizedEnergyType === 'Strom' ? verbrauchStrom : ''
  const normalizedGas = normalizedEnergyType === 'Gas' ? verbrauchGas : ''
  const activeConsumption = normalizedEnergyType === 'Strom' ? normalizedStrom : normalizedGas
  if (activeConsumption && (!/^\d+$/.test(activeConsumption) || Number(activeConsumption) <= 0)) {
    return { ok: false, code: 'invalid-consumption' }
  }

  const normalizedLocations =
    standorte === '1' ? '1' :
    ['2', '3', '4', '5', '2-5', '2–5'].includes(standorte) ? '2–5' :
    ['6', '6+'].includes(standorte) ? '6+' :
    ''

  if (!normalizedLocations) {
    return { ok: false, code: standorte ? 'invalid-locations' : 'locations-required' }
  }
  ...
  return {
    ...
    data: {
      ...
      telefon,
      plz,
      energieart: normalizedEnergyType,
      verbrauchStrom: normalizedStrom,
      verbrauchGas: normalizedGas,
      standorte: normalizedLocations,
      versorger,
      vertragslaufzeit,
      nachricht,
