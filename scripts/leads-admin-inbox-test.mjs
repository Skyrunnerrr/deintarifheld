...
import { ADMIN_INBOX_HTML, ADMIN_LOGIN_HTML } from '../lib/leads/admin-inbox-html.js'
...
function assertAuth() {
  ...
  assert.equal(isAdminAuthorized(fakeRequest({ authorization: 'Bearer cron-must-not-open-admin' })), false)
  assert.equal(isCronAuthorized(fakeRequest({ authorization: 'Bearer ops-secret-test' })), false)
  assert.equal(isCronAuthorized(fakeRequest({ authorization: 'Bearer cron-must-not-open-admin' })), true)
  assert.equal(isCronAuthorized(fakeRequest({ 'x-admin-secret': 'ops-secret-test' })), false)
...
function assertHtmlAndRoutes() {
  ...
  assert.match(ADMIN_LOGIN_HTML, /Ops-Geheimnis/)
  assert.doesNotMatch(ADMIN_LOGIN_HTML, /sessionStorage/)
  ...
  assert.match(list, /enforceAdminAccess/)
  ...
  assert.match(inbox, /isAdminAuthorized|enforceAdminAccess/)
  const del = readFileSync(join(root, 'app/api/admin/leads/delete/route.js'), 'utf8')
  assert.match(del, /enforceAdminAccess/)
