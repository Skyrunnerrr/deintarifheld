...
function assertLegalAlignmentDocs() {
  ...
  assert.match(pub, /LEGAL_TEXT_CODE_MISMATCH=NO/)
  ...
  assert.match(agb, /kein vertraglicher Anspruch auf eine automatische\s+Eingangsbestätigung/)
  assert.match(agb, /TDDDG/)
  assert.doesNotMatch(agb, /Auftragsnummer/)
  const ds = read('app/datenschutz/page.js')
  assert.match(ds, /reCAPTCHA/)
  assert.match(ds, /ProvenExpert/)
  assert.match(ds, /th_consent/)
  assert.match(ds, /dth_pe_withdraw_reload/)
  assert.doesNotMatch(ds, /als gelöscht gekennzeichnet/)
  assert.match(read('docs/deployment/PR6_DEPLOY_ORDER.md'), /AUTO_PRODUCTION_DEPLOY_ON_MAIN=NO/)
  ...
  console.log('PUBLIC_LEGAL_ALIGNMENT=PASS')
